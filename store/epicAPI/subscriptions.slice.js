...
export const api_v1_subscription_list = createAsyncThunk("subscriptions/api_v1_subscription_list", async payload => {
  const response = await apiService.api_v1_subscription_list(payload);
  return response.data;
});
export const api_v1_subscription_create = createAsyncThunk("subscriptions/api_v1_subscription_create", async payload => {
  const response = await apiService.api_v1_subscription_create(payload);
  return response.data;
});
export const api_v1_subscription_retrieve = createAsyncThunk("subscriptions/api_v1_subscription_retrieve", async payload => {
  const response = await apiService.api_v1_subscription_retrieve(payload);
  return response.data;
});
export const api_v1_subscription_update = createAsyncThunk("subscriptions/api_v1_subscription_update", async payload => {
  const response = await apiService.api_v1_subscription_update(payload);
  return response.data;
});
export const api_v1_subscription_partial_update = createAsyncThunk("subscriptions/api_v1_subscription_partial_update", async payload => {
  const response = await apiService.api_v1_subscription_partial_update(payload);
  return response.data;
});
export const api_v1_subscription_destroy = createAsyncThunk("subscriptions/api_v1_subscription_destroy", async payload => {
...
const subscriptionsSlice = createSlice({
  ...
  extraReducers: {
    ...
    [api_v1_subscription_retrieve.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending";
      }
    },
    [api_v1_subscription_retrieve.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
        state.entities = [...state.entities.filter(record => record.id !== action.payload.id), action.payload];
        state.api.loading = "idle";
      }
    },
    [api_v1_subscription_retrieve.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
        state.api.error = action.error;
        state.api.loading = "idle";
      }
    ...
    [api_v1_subscription_destroy.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending";
      }
    },
    [api_v1_subscription_destroy.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
        state.entities = state.entities.filter(record => record.id !== action.meta.arg?.id);
        state.api.loading = "idle";
      }
    },
    [api_v1_subscription_destroy.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
        state.api.error = action.error;
        state.api.loading = "idle";
      }
